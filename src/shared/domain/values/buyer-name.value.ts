...
export class BuyerName {
  ...
  public static create(firstName: string, lastName: string): BuyerName {
    firstName = (firstName ?? "").trim();
    lastName = (lastName ?? "").trim();
    return new BuyerName(firstName, lastName);
  }

  public static createv2(firstName: string, lastName: string): Result<AppNotification, BuyerName> {
    ...
    return Result.ok(new BuyerName(firstName, lastName));
  }
}